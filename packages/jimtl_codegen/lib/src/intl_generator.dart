...
import 'package:jimtl_codegen/src/generate_localized.dart';
...
class ArbBuilder extends GeneratorForAnnotation<GenerateArb> {
  ...
  generateForAnnotatedElement(Element element, ConstantReader annotation, BuildStep buildStep) {
    ...
    final arbDir = annotation.read('dir').stringValue;
    ...
    extraction.suppressLastModified = annotation.read('suppressLastModified').boolValue;
    extraction.suppressWarnings = false;

    final name = baseFileName ?? path.basenameWithoutExtension(inputId.path);

    final allMessages = extraction.parseFile(file, false);
    final extension = defaultFlavor == IntlDelegate.defaultFlavorName ? '_${defaultLocale}.arb' : '_${defaultFlavor}_${defaultLocale}.arb';
    var copyAssetId = inputId.changeExtension(extension);

    if (arbDir != '.') {
      final assetPath = path.join(arbDir, '$name$extension');
